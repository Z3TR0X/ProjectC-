    ...
    public partial class Form1 : Form
    {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == 0x84)
            {
                ...
                // Haut Gauche
                if (clientPoint.X <= tolerance && clientPoint.Y <= tolerance)
                { m.Result = (IntPtr)13; return; }

                // Haut Droit
                if (clientPoint.X >= this.Size.Width - tolerance && clientPoint.Y <= tolerance)
                { m.Result = (IntPtr)14; return; }

                // Bas Gauche
                if (clientPoint.X <= tolerance && clientPoint.Y >= this.Size.Height - tolerance)
                { m.Result = (IntPtr)16; return; }

                // Bas Droit
                if (clientPoint.X >= this.Size.Width - tolerance && clientPoint.Y >= this.Size.Height - tolerance)
                { m.Result = (IntPtr)17; return; }

                // Bord Gauche
                if (clientPoint.X <= tolerance)
                { m.Result = (IntPtr)10; return; }

                // Bord Droit
                if (clientPoint.X >= this.Size.Width - tolerance)
                { m.Result = (IntPtr)11; return; }

                // Bord Haut
                if (clientPoint.Y <= tolerance)
                { m.Result = (IntPtr)12; return; }

                // Bord Bas
                if (clientPoint.Y >= this.Size.Height - tolerance)
